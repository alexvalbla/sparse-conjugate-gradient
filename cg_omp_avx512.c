...
struct csr_matrix_t *load_mm(FILE * f)
{
	...
	// Ap, Aj aligned on 32-byte boundary
	// Ax aligned on 64-byte boundary
	...
	int *Ap = aligned_alloc(32, aligned_alloc_size);

	aligned_alloc_size = 2 * nnz * sizeof(int);
	mod = aligned_alloc_size % 32;
	aligned_alloc_size += 32-mod;
	int *Aj = aligned_alloc(32, aligned_alloc_size);

	aligned_alloc_size = 2 * nnz * sizeof(double);
	mod = aligned_alloc_size % 64;
	aligned_alloc_size += 64-mod;
	double *Ax = aligned_alloc(64, aligned_alloc_size);
...
void sp_gemv2(const struct csr_matrix_t *A, const double *x, double *y)
{
	...
	int *Aj = A->Aj;
	double *Ax = A->Ax;
	int rem; // remainder after modulus 8
	#pragma omp parallel for private(rem)
	for (int i = 0; i < n; i++) {
		y[i] = 0;
		// we will compute elements by blocs of 8
		rem = (Ap[i+1]-Ap[i]) & 7; // modulus 8
		__m256i vec_indexes; // column indexes of elements on this line
		__m512d vec_A; // elements of this line of A
		__m512d vec_x; // corresponding elements of x
		__m512d vec_res = _mm512_setzero_pd(); // where the results will be added
		#pragma omp simd
		for(int u = Ap[i]; u < Ap[i+1]-7; u += 8){
			vec_indexes = _mm256_loadu_si256( (__m256i const *) &Aj[u]);
			vec_A = _mm512_loadu_pd(&Ax[u]);
			vec_x = _mm512_i32gather_pd(vec_indexes, x, sizeof(double));
			vec_res = _mm512_fmadd_pd(vec_A, vec_x, vec_res);
		}
		double tmp[8] __attribute__((aligned(64)));
		_mm512_store_pd(tmp, vec_res);
		y[i] += tmp[0];
		y[i] += tmp[1];
		y[i] += tmp[2];
		y[i] += tmp[3];
...
void sp_gemv3(const struct csr_matrix_t *A, const double *x, double *y)
{
	...
	#pragma omp parallel for private(l, rem, start)
	for (int i = 0; i < n; i++) {
		y[i] = 0;
		l = Ap[i+1] - Ap[i];
		start = Ap[i];
		rem = Ap[i] & 7; // modulus 8
		// small loop to get to an aligned address
		if(rem > 0 && l >= 8-rem){
			for(int u = 0; u < 8-rem; u++){
				int j = Aj[Ap[i]+u];
		...
		__m512d vec_x; // corresponding elements of x
		__m512d vec_res = _mm512_setzero_pd(); // where the results will be added
		for(int u = start; u < Ap[i+1]-7; u += 8){
			vec_indexes = _mm256_load_si256( (__m256i const *) &Aj[u]);
			vec_A = _mm512_load_pd(&Ax[u]);
			vec_x = _mm512_i32gather_pd(vec_indexes, x, sizeof(double));
			vec_res = _mm512_fmadd_pd(vec_A, vec_x, vec_res);
		}
		double tmp[8] __attribute__((aligned(64)));
