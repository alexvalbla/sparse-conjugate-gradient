...
void cg_solve(const struct csr_matrix_t *A, const double *b, double *x, const double epsilon, int num_of_proc, int my_rank)
{
	 ...
	 * -all processes allocate memory for vector x (except process 0 -> xallocated in main()...)
	...
	double * tmp = (double *)malloc(sizeof(double) * num_of_proc * 2);
	//tmp will be used to gather partial results from all process

	double scalar_bloc[2];
	...
	while(norm_r > epsilon){
		double old_rz = rz;
		/* each process calculates part of pq */
		sp_gemv_mpi(&A_sub, p, q, my_number_of_lines);
		double partial_pq = dot(my_number_of_lines, p+my_starting_index, q);

		MPI_Allgather(&partial_pq, 1, MPI_DOUBLE, tmp, 1, MPI_DOUBLE, MPI_COMM_WORLD);

		/* now compute pq fully */
		double pq = 0.;
		for(int i = 0; i < num_of_proc; i++)
			pq += tmp[i];
		...
		MPI_Allgather(scalar_bloc, 2, MPI_DOUBLE, tmp, 2, MPI_DOUBLE, MPI_COMM_WORLD);

		/* now compute rz and norm_r fully */
		rz = 0.;
		norm_r = 0.;
		...
		//table2 gives the starting indexe for the section communicated by each process
